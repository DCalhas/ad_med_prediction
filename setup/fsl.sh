...
for f in $(ldd ${FSLDIR}/bin/flirt | awk '{print $3}' | grep -v '^(' | grep -v '^/lib64'); do
        cp -v "$f" ${ENVNAME}/lib/
done

cp -v ${FSLDIR}/lib/libblas.so.3 ${ENVNAME}/lib/.

mv ${FSLDIR}/bin/bet ${ENVNAME}/bin/.
mv ${FSLDIR}/bin/bet2 ${ENVNAME}/bin/.
mv ${FSLDIR}/bin/flirt ${ENVNAME}/bin/.
mv ${FSLDIR}/bin/fslmaths ${ENVNAME}/bin/.
mv ${FSLDIR}/bin/fslstats ${ENVNAME}/bin/.
mv ${FSLDIR}/bin/fslval ${ENVNAME}/bin/.
mv ${FSLDIR}/bin/remove_ext ${ENVNAME}/bin/.
mv ${FSLDIR}/bin/imtest ${ENVNAME}/bin/.
