...
def nearest_interp_dates(query_dates, known_dates, known_values, fill_value=101):
	qd = np.asarray(query_dates).astype("datetime64[ns]")
	kd = np.asarray(known_dates).astype("datetime64[ns]")
	y	= np.asarray(known_values)

	valid = ~np.isnat(kd)
	kd, y = kd[valid], y[valid]

	if kd.size == 0 or y.size == 0:
		return np.full(qd.shape, fill_value, dtype=y.dtype if y.size else float)

	order = np.argsort(kd)
	kd, y = kd[order], y[order]

	out = np.full(qd.shape, fill_value, dtype=y.dtype)

	q_valid = ~np.isnat(qd)
	if not q_valid.any():
		return out

	x	= kd.astype(np.int64)
	xi = qd[q_valid].astype(np.int64)

	idx = np.searchsorted(x, xi, side="left")
	left	= np.clip(idx - 1, 0, len(x) - 1)
	right = np.clip(idx,		 0, len(x) - 1)

	choose_left = np.abs(xi - x[left]) <= np.abs(x[right] - xi)
	nearest = np.where(choose_left, left, right)

	out[q_valid] = y[nearest]
	return out
...
class _GetTransitions():
	...
	def __call__(self, dataset_dir="/ceph/hpc/data/s25r10-02-users/calhasd/adni", state="ON"):
		...
		for pat in pat_curves:
			...
			cond_measures=pat_curves[pat].cond_measures[:n,:]
			...
			if np.all(np.diff(progression) == 0):
				...
				pat_transitions[pat].append(self.transition_class(pat, final_time, start_age, gender, s, s, final_time, 101*np.ones((1,self.transition_class.n_features), dtype=np.int32)))
				continue
			...
			for c in range(len(changes)-1):
				...
				start_age=age_weeks[idx]
				next_age=age_weeks[next_idx]

				if(c==0):
					measures=101*np.ones((1,self.transition_class.n_features), dtype=np.int32)
				else:
					measures=cond_measures[:idx,:]
				...
				if len(pat_transitions[pat]) > 0:
					...
					if orig.x == prev.orig and dest.x == prev.dest:
						pat_transitions[pat][-1] = self.transition_class(pat,prev.duration + duration,start_age,gender,orig,dest,time, measures)
						continue
				else:
					duration = int(tt[next_idx])

				pat_transitions[pat].append(self.transition_class(pat, duration, start_age, gender, orig, dest, time, measures))
			...
			if len(pat_transitions[pat]) > 0:
				...
				if final_time > last_time:
					...
					pat_transitions[pat].append(self.transition_class(pat, final_time - last_time, next_age, gender, last_state, last_state, final_time, measures))
			else:
				final_time = int(tt[-1])
				final_age=age_weeks[0]
				s = self.state_class(progression[0])
				pat_transitions[pat].append(self.transition_class(pat, final_time, final_age, gender, s, s, final_time, 101*np.ones((1,self.transition_class.n_features), dtype=np.int32)))
